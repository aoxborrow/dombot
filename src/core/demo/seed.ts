...
const SUFFIXES = ['hq', 'app', 'labs', 'studio', 'co', 'works', 'shop', 'hub'];
const PEOPLE = [
  ['ada', 'lindqvist'],
  ['marco', 'reyes'],
  ['priya', 'natarajan'],
...
function makeLabel(r: () => number): string {
  const roll = r();
  if (roll < 0.42) return pick(r, ADJECTIVES) + pick(r, NOUNS);
  if (roll < 0.72) return pick(r, SINGLE);
  if (roll < 0.84) return `${pick(r, ADJECTIVES)}-${pick(r, NOUNS)}`;
  if (roll < 0.94) return pick(r, SINGLE) + pick(r, SUFFIXES);
  const [first, last] = pick(r, PEOPLE);
  return r() < 0.5 ? `${first}${last}` : `${first}-${last}`;
}
  ...
  accounts.forEach((account, ai) => {
    for (let i = 0; i < quotas[ai]; i++) {
      ...
      for (let tries = 0; tries < 50; tries++) {
        const candidate = `${makeLabel(r)}.${pickTld(r, account.registrar)}`;
  ...
  const folders: DemoFolder[] = [
    {
      ...
      domains: names
        .filter((n) => PEOPLE.some(([f]) => n.startsWith(f)))
        .concat(byTld(['studio', 'design', 'cafe'])),
    },
    {
      name: 'Side projects',
      color: 'violet',
